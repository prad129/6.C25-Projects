...
using Plots, LinearAlgebra, Triangulate, GeometryBasics

# ╔═╡ c5a9c92c-a9ad-4fa8-a15f-2fd39d70d60c
# ╠═╡ disabled = true
#=╠═╡
begin
gr()

# Define the line segments as pairs of points ((x1, y1), (x2, y2))
segments = [
    ((1.0, 1.0), (6.0, 5.0)),
    ((3.0, 6.0), (8.0, 3.0)),
    ((2.0, 4.0), (7.0, 1.0)),
...
# Define a function that draws the current state of the algorithm
function plot_sweep_line(sweep_pos, segments, active_segments)
    plot(xlims=(0, 10), ylims=(0, 7), legend=false, title="Sweep Line Algorithm")

    # Plot each segment
    for ((x1, y1), (x2, y2)) in segments
        plot!([x1, x2], [y1, y2], lw=2, label=false, color=:blue)
    end

    # Draw the sweep line at the current position
    plot!([sweep_pos, sweep_pos], [0, 7], lw=2, linestyle=:dash, label=false, color=:red)

    # Highlight active segments in green
    for ((x1, y1), (x2, y2)) in active_segments
        plot!([x1, x2], [y1, y2], lw=3, label=false, color=:green)
    end
end

# Function to simulate the sweep line process
function sweep_line_animation(segments)
    anim = Animation() # Create an animation object
    sorted_events = sort([s[1][1] for s in segments]) # Sort by x-coordinates of segment start points
    
    active_segments = [] # Store active segments
    
    for sweep_pos in sorted_events
        # Add new segments that start at this sweep position
        for seg in segments
            if seg[1][1] == sweep_pos
                push!(active_segments, seg)
            end
        end

        # Draw the current state of the sweep line
        plot_sweep_line(sweep_pos, segments, active_segments)

        # Save each frame
        frame(anim)

        # Simulate sweeping past the current segments
        active_segments = [s for s in active_segments if s[2][1] > sweep_pos]
    end
    
    # Create and save the gif animation
    gif(anim, "sweep_line_algorithm.gif", fps=2)
end

# Run the sweep line animation
sweep_line_animation(segments)
end
  ╠═╡ =#
...
# ╔═╡ fb296955-bf5e-424a-a8bd-c780b91e39dc
md"""
# Sweep Line

Many geometric problems can be solved using sweep line algorithms. The idea in such algorithms is to represent an instance of the problem as a set of events that correspond to points in the plane. The events are processed in increasing order according to their x or y coordinates. Here's an example animation that shows how this algorithm works. We highlight lines that are currently being processed in green.

"""

# ╔═╡ e3d56048-60cf-44aa-beff-c9c04203a0f0
begin
gr()


function generate_random_segments(num_segments::Int, x_range::Tuple{Float64, Float64}, y_range::Tuple{Float64, Float64})
    segments = []
    for _ in 1:num_segments
        x1, y1 = rand(x_range[1]:x_range[2]), rand(y_range[1]:y_range[2])
        x2, y2 = rand(x_range[1]:x_range[2]), rand(y_range[1]:y_range[2])
...
function plot_sweep_line(sweep_pos, segments, active_segments)
    ...
    # active segments get highlighted in green
    for ((x1, y1), (x2, y2)) in active_segments
        plot!([x1, x2], [y1, y2], lw=3, label=false, color=:green)
    end
end
...
function sweep_line_animation(segments)
    ...
    gif(anim, "sweep_line_algorithm_random.gif", fps=1)
end
...
segments = generate_random_segments(num_segments, (0.0, 10.0), (0.0, 7.0))

sweep_line_animation(segments)

end

# ╔═╡ 265682ed-c53b-40bd-9b6a-e9d9a6a19d3c
md"""
This algorithm might seem rather unimpressive at first, but it allows us to solve many interesting problems. Here are a few.

"""

# ╔═╡ a90b3fae-c363-47e2-8b3e-68c033d6be79
md"""
# Closest Pair of Points

Aircraft proximity detection is essential in air traffic control systems. Airports and control towers must continuously track the position of all planes and check which pairs of aircraft are dangerously close to each other. The sweep line algorithm could help efficiently process the locations of aircraft and find those closest to one another, preventing mid-air collisions or planning optimal paths for takeoff and landing. 

Given a set of points representing the location of each plane, we must find the closest pair of planes.

A naive way to go about doing this would be to look at every pair of planes, but we can do better!

"""

# ╔═╡ 6e8338e5-ed37-47b7-9159-43807e85d263
begin


eleven_nodes = [randn(2) for i = 1:10]
...
scatter(first.(eleven_nodes), last.(eleven_nodes), 
        aspect_ratio=:equal, 
        xlims=(-3, 3), 
        ylims=(-3, 3),
        label=false)
...
anim = @animate for sweep_pos in -3:0.1:3
    ...
    plot!([sweep_pos, sweep_pos], [-3, 3], lw=2, linestyle=:dash, color=:red, label=false)

    # Variables to track the closest pair of points
    closest_pair = nothing
    min_dist = Inf

    # Find the closest pair of points within the sweep line region
    for i in 1:length(eleven_nodes)
        for j in i+1:length(eleven_nodes)
            if eleven_nodes[i][1] <= sweep_pos && eleven_nodes[j][1] <= sweep_pos  # Only compare points left of the sweep line
                dist = compute_euclidean_distance(eleven_nodes[i], eleven_nodes[j])
                if dist < min_dist
                    min_dist = dist
                    closest_pair = (eleven_nodes[i], eleven_nodes[j])
                end
            end
        end
    end

    # Highlight the closest pair of points if found
    if closest_pair != nothing
        plot!([closest_pair[1][1], closest_pair[2][1]], 
              [closest_pair[1][2], closest_pair[2][2]], 
              lw=2, color=:blue, label=false)

        # Plot the closest pair points as larger blue dots
        scatter!([closest_pair[1][1], closest_pair[2][1]], 
                 [closest_pair[1][2], closest_pair[2][2]], 
                 color=:blue, ms=8, label=false)
    end
end


gif(anim, "sweeping_line_closest_pair.gif", fps=15)

	
end

# ╔═╡ 8c0c331f-a2f5-4825-9c5b-521e78dfd973
html"""
<img src="https://images.ctfassets.net/piwi0eufbb2g/QBKnjI8xjQIg3Jj1V9qe1/a71f79c28504113da1a9dbda2f909c0f/closest.png"
width = "1000">
"""

# ╔═╡ 47c3b28e-1866-4f37-8093-d53f47f78860
md"""
...
# ╔═╡ 0f402ebb-ccef-4831-b3c4-bf5651b90d13
md"""
...
![Jarvis_march_convex_hull](https://upload.wikimedia.org/wikipedia/commons/2/28/Cvx_hull_Jarvis_steps_ND.gif)
(from <https://commons.wikimedia.org/wiki/File:Cvx_hull_Jarvis_steps_ND.gif>)
"""
...
# ╔═╡ b23ef371-8bde-453c-a2d6-a634a470519b
# Check if point c is clockwise-oriented with respect to a -> b -> c
function clockwise_oriented(a, b, c)
    return (b[1] - a[1]) * (c[2] - a[2]) - (b[2] - a[2]) * (c[1] - a[1]) < 0
end

# ╔═╡ a90cc361-046a-4e0f-bfd5-64827df550a9
clockwise_oriented([0,0], [1,1], [2,3])

# ╔═╡ 0e6004e8-66c4-4d65-8c07-f34d0d442281
clockwise_oriented([0,0], [1,1], [3,2])
...
# ╔═╡ b4ccb976-d4d6-423d-a1ef-a961c4d80453
md"""
For the Jarvis algorithm, we use the following steps instead:
...
# ╔═╡ cd6af2c9-7b9d-47a8-a854-743db1225a2e
begin
	t_nodes = [randn(2) for i = 1:10]
	...
	function jarvis_march(points)
	    ...
	    leftmost_idx = argmin(first.(points))
	    point_on_hull = points[leftmost_idx]
	    current_point = point_on_hull
	
	    while true
	        push!(hull, current_point)
	
	        next_point = points[1]

			for p in points
	            if next_point == current_point || cross_product(current_point, next_point, p) > 0
	                next_point = p
	            end
	        end
	        ...
	        if current_point == point_on_hull
	            break
	        end
	    end
	...
	function animate_jarvis_march(points, hull)
	    anim = @animate for i in 1:length(hull)
	        scatter(first.(points), last.(points), 
	                aspect_ratio=:equal, 
	                xlims=(-3, 3), 
	                ylims=(-3, 3),
	                label=false, 
	                title="Jarvis March Convex Hull Animation")
	
	        if i > 1
	            plot!(first.(hull[1:i]), last.(hull[1:i]), lw=2, color=:blue, label=false)
	        end
	
	        if i < length(hull)
	            plot!([hull[i][1], hull[i+1][1]], [hull[i][2], hull[i+1][2]], lw=2, linestyle=:dash, color=:red, label=false)
	        end
	
	        scatter!([hull[i][1]], [hull[i][2]], color=:green, ms=8, label=false)
    		plot!([hull[end][1], hull[1][1]], [hull[end][2], hull[1][2]], lw=2, color=:blue, label=false)

	    end
	    gif(anim, "jarvis_march.gif", fps=1)
	end
	
	hull = jarvis_march(t_nodes)
	
	animate_jarvis_march(t_nodes, hull)
...
manifest_format = "2.0"
project_hash = "276db6bfdc24bea258936408ace17c55be939d71"
